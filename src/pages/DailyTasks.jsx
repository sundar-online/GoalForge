...
export const DailyTasks = () => {
  const { addTask, deleteTask, logTaskTime } = useAppContext();
  const { todayTasks, completedTasksCount: doneCount, totalTasksCount: total, toggleTaskComplete, isTaskDone } = useTasks();

  const [isAdding, setIsAdding] = useState(false);
  const [showLog, setShowLog] = useState(null);
  
  const defaultTask = { title: '', targetTime: 30, priority: 'Medium', type: 'daily', completionType: 'time', targetDate: TODAY(), startDate: TODAY(), endDate: TODAY() };
      ...
      {isAdding && (
        <form onSubmit={submit} style={{ background: 'var(--bg-card)', borderRadius: 20, padding: '20px 22px', boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column', gap: 14, border: '1px solid var(--border-light)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div>
              <p style={{ margin: '0 0 6px', fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Completion Type</p>
              <div style={{ display: 'flex', gap: 8, background: 'var(--bg-input)', padding: 4, borderRadius: 12 }}>
                {['time', 'check'].map(cType => (
                  <button type="button" key={cType} onClick={() => setNewTask({ ...newTask, completionType: cType })}
                    style={{ flex: 1, padding: '8px 0', border: 'none', borderRadius: 9, fontSize: 12, fontWeight: 700, cursor: 'pointer', background: newTask.completionType === cType ? 'var(--bg-card)' : 'transparent', color: newTask.completionType === cType ? 'var(--text-main)' : 'var(--text-muted)', boxShadow: newTask.completionType === cType ? '0 2px 6px rgba(0,0,0,0.06)' : 'none' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            {newTask.completionType === 'time' && (
              <div>
                <p style={{ margin: '0 0 6px', fontSize: 10, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Duration (mins)</p>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    <button 
                      onClick={() => task.completionType === 'check' ? toggleTaskComplete(task.id) : null}
                      style={{ width: 26, height: 26, borderRadius: 8, background: tDone ? '#22c55e' : 'var(--bg-input)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.2s', border: 'none', cursor: task.completionType === 'check' ? 'pointer' : 'default' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
                        ...
                        {task.currentStreak > 0 && typeStr === 'daily' && (
                          <span style={{ fontSize: 10, fontWeight: 800, color: '#f59e0b', background: 'rgba(245,158,11,0.1)', padding: '2px 6px', borderRadius: 4 }}>
                            🔥 {task.currentStreak}
                          </span>
                        )}
                      </div>
                      <p style={{ margin: 0, fontSize: 15, fontWeight: 700, color: tDone ? '#22c55e' : 'var(--text-main)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', transition: 'all 0.2s', textDecoration: (tDone && task.completionType === 'check') ? 'line-through' : 'none' }}>
                        {task.title}
                      </p>
                      <p style={{ margin: '3px 0 0', fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                        {task.completionType === 'check' ? (tDone ? 'Completed' : 'Pending') : `${task.timeSpent || 0}m / ${task.targetTime}m target`}
                      </p>
                    </div>

                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {!tDone && task.completionType === 'time' && (
