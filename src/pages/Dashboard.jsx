...
export const Dashboard = ({ setView }) => {
  const { 
    ...
    theme, toggleTheme,
    focusHrs, focusMins, focusDelta,
    totalItemsCount, completedItemsCount, quote
  } = useDashboard();
  
  const { displayName, signOut, user } = useAuth();
  const { topStreaks } = useStreak(goals, tasks);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, paddingBottom: 100 }}>
      ...
      {/* Heatmap */}
      <WeeklyHeatmap taskLogs={taskLogs} />

      {/* Task Stats Row */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
        {[
          { label: 'Units', val: totalItemsCount, icon: <Zap size={14} color="var(--accent-blue)" /> },
          { label: 'Done', val: completedItemsCount, icon: <CheckCircle2 size={14} color="#22c55e" /> },
          { label: 'Peak', val: Math.max(totalItemsCount, completedItemsCount), icon: <Clock size={14} color="#f97316" /> },
        ].map((s, i) => (
      ...
      <section>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          {goals.slice(0, 2).map(goal => {
            ...
            const habitsDone = (goal.habits || []).filter(h => h.completed || (h.timeSpent || 0) >= (h.targetTime || 15)).length;
            return (
