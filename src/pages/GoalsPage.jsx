...
import { 
  Target, Plus, ChevronDown, ChevronUp, Trash2, Clock, Check, Layers, 
  Calendar, RotateCcw, AlertCircle, History 
} from 'lucide-react';
import { TODAY, addDays, getDaysLeft, formatDate } from '../utils/dateUtils';

// ── Extend Deadline Modal/Section ──────────────────────────
const ExtendDeadlineSection = ({ goal, extendDeadline }) => {
  ...
  const handleExtend = (days) => {
    const current = goal.deadline || TODAY();
    const extended = addDays(current, days);
    extendDeadline(goal.id, extended);
    setShowOptions(false);
  };

  const handleSetDate = (e) => {
    e.preventDefault();
    if (newDate <= TODAY()) return alert("New deadline must be in the future!");
    extendDeadline(goal.id, newDate);
    setNewDate('');
    setShowOptions(false);
  };

  return (
    <div style={{ marginTop: 8, padding: '12px', background: 'var(--bg-input)', borderRadius: 16, border: '1px solid var(--border-light)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <p style={{ margin: 0, fontSize: 13, fontWeight: 700, color: 'var(--text-main)' }}>
              Deadline: {goal.deadline ? formatDate(goal.deadline) : 'Not Set'}
            </p>
            <p style={{ margin: 0, fontSize: 11, color: daysLeft <= 2 ? '#ef4444' : 'var(--text-muted)', fontWeight: 600 }}>
              {daysLeft !== null ? `${daysLeft} days remaining` : 'No time limit'}
            </p>
          </div>
        </div>
        <button onClick={() => setShowOptions(!showOptions)}
          style={{ background: 'var(--bg-card)', border: '1px solid var(--border-light)', padding: '6px 12px', borderRadius: 10, fontSize: 12, fontWeight: 700, cursor: 'pointer', color: 'var(--text-main)' }}>
          {showOptions ? 'Cancel' : 'Extend'}
        </button>
      </div>

      {showOptions && (
        <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 10, animation: 'fadeIn 0.2s ease-out' }}>
          <div style={{ display: 'flex', gap: 8 }}>
            {[3, 7, 14].map(d => (
              <button key={d} onClick={() => handleExtend(d)}
                style={{ flex: 1, padding: '8px', borderRadius: 10, border: 'none', background: 'var(--bg-card)', color: 'var(--text-main)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
                +{d}d
              </button>
            ))}
          </div>
          <form onSubmit={handleSetDate} style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 8 }}>
            <input type="date" value={newDate} onChange={e => setNewDate(e.target.value)} required min={addDays(TODAY(), 1)}
              style={{ background: 'var(--bg-card)', border: '1px solid var(--border-light)', borderRadius: 10, padding: '8px 12px', fontSize: 12, color: 'var(--text-main)', outline: 'none' }} />
            <button type="submit" style={{ background: 'var(--accent-blue)', color: '#fff', border: 'none', borderRadius: 10, padding: '8px 16px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
              Set Date
            </button>
          </form>
          
          {goal.extensions?.length > 0 && (
            <div style={{ marginTop: 4, padding: '8px', background: 'rgba(0,0,0,0.02)', borderRadius: 10 }}>
              <p style={{ margin: '0 0 6px', fontSize: 10, fontWeight: 800, color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4 }}>
                <History size={12} /> EXTENSION HISTORY
              </p>
              {goal.extensions.map((ex, i) => (
                <p key={i} style={{ margin: '2px 0', fontSize: 10, color: 'var(--text-muted)' }}>
                  {formatDate(ex.old_date)} → {formatDate(ex.new_date)} (on {formatDate(ex.extended_on)})
                </p>
              ))}
            </div>
          )}
...
const LogTimeModal = ({ habit, goalId, onClose, logHabitTime }) => {
  ...
  return (
    <div 
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20, backdropFilter: 'blur(4px)' }}
      onClick={onClose}
    >
      <div 
        style={{ background: 'var(--bg-card)', borderRadius: 24, padding: '28px 24px', width: '100%', maxWidth: 340, boxShadow: 'var(--shadow-float)', border: '1px solid var(--border-light)' }}
...
const HabitRow = ({ habit, goalId, logHabitTime, deleteHabit, toggleHabitCheck, updateHabitCount }) => {
  ...
  else if (isCount) done = (current || 0) >= (target || 10);
  else done = (current || 0) >= (target || 15);
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
            <button
               ...
               style={{ width: 32, height: 32, borderRadius: 10, background: done ? '#22c55e' : 'var(--bg-card)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all 0.3s', boxShadow: '0 2px 4px rgba(0,0,0,0.02)' }}
            >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 2, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>
                  {isCheck ? 'Check-based' : isCount ? `${current} / ${target} units` : `${current}m / ${target}m`}
                </span>
              </div>
            </div>
          </div>
...
export const GoalsPage = () => {
  const { 
    goals: allGoalsFromContext, addGoal, deleteGoal, addHabit, deleteHabit, 
    logHabitTime, toggleHabitCheck, updateHabitCount, extendGoalDeadline 
  } = useAppContext();
  const { doneGoalsCount: doneGoals, activeGoalsCount: activeGoals, avgProgress, goalsWithStatus } = useGoals(allGoalsFromContext);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      {showAddGoal && (
        <form onSubmit={submitGoal} style={{ background: 'var(--bg-card)', borderRadius: 20, padding: '20px', boxShadow: 'var(--shadow-sm)', display: 'flex', flexDirection: 'column', gap: 14, border: '1px solid var(--border-light)' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {['ANY', 'ALL', 'CUSTOM'].map(m => (
                <div key={m} onClick={() => setNewGoal({ ...newGoal, mode: m })}
                  style={{ flex: 1, padding: '10px', borderRadius: 12, cursor: 'pointer', textAlign: 'center', background: newGoal.mode === m ? 'var(--accent-blue-light)' : 'var(--bg-input)', color: newGoal.mode === m ? 'var(--accent-blue)' : 'var(--text-muted)', border: `2px solid ${newGoal.mode === m ? 'var(--accent-blue)' : 'transparent'}` }}>
          ...
          <button type="submit" style={{ background: 'var(--bg-dark-elem)', color: 'var(--text-inverted)', border: 'none', borderRadius: 12, padding: '13px', fontWeight: 700, fontSize: 14, cursor: 'pointer' }}>
            Create Goal
          </button>
        </form>
      )}
      ...
      {goalsWithStatus.map(goal => {
        ...
        const doneToday = goal.doneToday;

        return (
          <div key={goal.id} style={{ background: 'var(--bg-card)', borderRadius: 22, overflow: 'hidden', boxShadow: 'var(--shadow-sm)', border: `2px solid ${doneToday ? '#22c55e' : 'var(--border-light)'}` }}>
            {doneToday && (
              <div style={{ background: '#22c55e', padding: '6px', textAlign: 'center' }}>
                <p style={{ margin: 0, fontSize: 11, fontWeight: 700, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
            ...
            <div style={{ padding: '18px 20px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 14 }}
              ...
              <div style={{ position: 'relative', width: 68, height: 68, flexShrink: 0 }}>
                <svg width="68" height="68" viewBox="0 0 68 68">
                  ...
                  <circle cx="34" cy="34" r={R} fill="none" stroke={doneToday ? '#22c55e' : 'var(--text-main)'} strokeWidth="7"
                    strokeDasharray={CIRC} strokeDashoffset={goalOffset} strokeLinecap="round" transform="rotate(-90 34 34)" />
                </svg>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, background: 'var(--bg-input)', color: 'var(--text-main)', padding: '3px 9px', borderRadius: 999 }}>{mode} {mode === 'CUSTOM' && `(${goal.minHabits})`}</span>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4 }}>
                  <p style={{ margin: 0, fontSize: 11, color: 'var(--text-muted)' }}>{goal.deadline || 'No deadline'}</p>
                  {(goal.streak || 0) > 0 && <span style={{ fontSize: 11, fontWeight: 700, color: '#f97316' }}>🔥 {goal.streak}d</span>}
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <button onClick={e => { e.stopPropagation(); deleteGoal(goal.id); }}
                  style={{ width: 32, height: 32, borderRadius: 10, border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)' }}>
                  <Trash2 size={14} />
                </button>
            ...
            {isOpen && (
              <div style={{ borderTop: '1px solid var(--border-light)', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <ExtendDeadlineSection goal={goal} extendDeadline={extendGoalDeadline} />
                <button onClick={() => setShowAddHabit(goal.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px', borderRadius: 12, background: 'var(--accent-blue-light)', border: '1.5px dashed var(--accent-blue)', cursor: 'pointer', fontSize: 13, fontWeight: 700, color: 'var(--accent-blue)' }}>
                  <Plus size={16} /> Add Habit
                </button>
                {showAddHabit === goal.id && (
                  <form onSubmit={e => submitHabit(e, goal.id)} style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <input autoFocus required type="text" value={newHabit.title} onChange={e => setNewHabit({ ...newHabit, title: e.target.value })} placeholder="Habit name"
                      style={{ border: '2px solid var(--accent-blue)', borderRadius: 12, padding: '10px', fontSize: 14, fontWeight: 600, background: 'var(--bg-card)', color: 'var(--text-main)', outline: 'none' }} />
                    <div style={{ display: 'flex', gap: 8 }}>
                      <select value={newHabit.type} onChange={e => setNewHabit({ ...newHabit, type: e.target.value })} style={{ flex: 1, border: '1px solid var(--border-med)', borderRadius: 12, padding: '10px', background: 'var(--bg-card)', color: 'var(--text-main)', outline: 'none' }}>
                        <option value="time">⏱️ Time</option>
                        <option value="check">✅ Check</option>
                        <option value="count">🔢 Count</option>
                      </select>
                    </div>
                    <button type="submit" style={{ padding: '10px', borderRadius: 12, background: 'var(--accent-blue)', color: '#fff', fontWeight: 700, cursor: 'pointer', border: 'none' }}>Add</button>
                  </form>
                )}
              </div>
            )}
      ...
      {goalsWithStatus.length === 0 && !showAddGoal && (
